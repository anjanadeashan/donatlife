...
<body style="background-image: url(../Images/BGPic.jpg); background-size: cover;" class=" m-0 border-0">
  ...
  if (array_key_exists("o_fill_form", $_POST)) {
    ...
    if ($conn->query($sql) === TRUE) {
      echo "<script>Swal.fire(
                'Organ Bank Update',
                'Organ Bank Update successfully',
                'success'
              ).then((result) => {
                if (result.isConfirmed) {
      ...
      echo "<script>Swal.fire(
                'Organ Bank Update',
                'Organ Bank Update failed',
                'error'
              ).then((result) => {
                if (result.isConfirmed) {
                  window.location.href = '../OrganForm2.php';
                }
              })</script>";
    }
  }
